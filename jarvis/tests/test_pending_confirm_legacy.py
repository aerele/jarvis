"""Tests for jarvis.chat.pending_confirm_legacy - the Redis token store kept for
dual-read until PR-4 (the ``pending_confirm`` facade mints on it when the
``jarvis_pa_chat_cards`` flag is 0, or the conversation can't host a pending
action). Redis internals only: mint/peek/consume and the args_hash that binds a
token to the exact call. The facade's behaviour is ``test_pending_confirm``.
"""
...
from jarvis.chat import pending_confirm_legacy as pending_confirm
