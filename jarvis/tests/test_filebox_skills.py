...
class TestSkillEditorEndpoints(_Base):
	...
	def test_the_lists_skip_the_file_box_columns_before_their_migrate(self):
		...
		with as_user(OWNER), hide_fields({SKILL: new}):
...
class TestGetSkillGate(_Base):
	...
	def test_a_non_file_box_get_skill_never_reads_the_new_columns(self):
		...
		get_value, get_all = frappe.db.get_value, frappe.get_all
...
def hide_fields(fields_by_doctype: dict):
	"""Patch ``Meta.has_field`` so these doctypes lack these fields. Class-level, not
	per instance: Frappe 15's set_user() clears the request cache, so get_meta()
	hands back a fresh Meta after every as_user()."""
	from frappe.model.meta import Meta

	has = Meta.has_field

	def has_field(meta, fieldname):
		return fieldname not in fields_by_doctype.get(meta.name, ()) and has(meta, fieldname)

	return patch.object(Meta, "has_field", has_field)
...
def before_the_migrate():
	...
	with contextlib.ExitStack() as stack:
		stack.enter_context(hide_fields(_NEW_FIELDS))
