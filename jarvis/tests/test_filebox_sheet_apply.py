...
class TestResume(_ApplyBase):
	...
	def test_one_resume_quotes_every_outcome_and_every_answer(self):
		...
		self.assertIn("A master listed above as skipped: stop", head)
		self.assertIn("was never proposed - propose it now with create_doc", head)
		self.assertIn("not a re-ask", head)
		self.assertIn("decides to create a record: propose that record now with create_doc", head)
